...
sys.path.append(os.path.dirname(os.path.abspath(__file__))[:-6])
import torch
...
def main(args):
    ...
    if args.gt:
        ...
        trainer = import_str(cfg.trainer.type)(
            **cfg.trainer,
            num_timesteps=dataset.num_img_timesteps*2,
            model_config=cfg.model,
            num_train_images=len(dataset.train_image_set)*2,
            num_full_images=len(dataset.full_image_set)*2,
            test_set_indices=dataset.test_timesteps,
            scene_aabb=dataset.get_aabb().reshape(2, 3),
            device=device
        )
