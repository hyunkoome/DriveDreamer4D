...
class AffineTransform(nn.Module):
    ...
    def forward(self, image_infos):
        if "img_idx" in image_infos and (not self.in_test_set or self.ref_flag):
...
class DeformNetwork_MIX(nn.Module):
    def __init__(self, D=8, W=256, input_ch=3, output_ch=59, x_multires=10, t_multires=10,c_multires=10):
        super(DeformNetwork_MIX, self).__init__()
        self.D = D
        self.W = W
        self.input_ch = input_ch
        self.output_ch = output_ch
        ...
        self.embed_time_fn, time_input_ch = get_embedder(self.t_multires, 1)
        self.embed_fn, xyz_input_ch = get_embedder(self.x_multires, 3)
        self.embed_cond_fn, c_input_ch = get_embedder(self.c_multires, 7)
        self.input_ch = xyz_input_ch + time_input_ch+c_input_ch
    ...
    def forward(self, x, t,c):
        t_emb = self.embed_time_fn(t)
        x_emb = self.embed_fn(x)
        c_emb = self.embed_cond_fn(c)
        h = torch.cat([x_emb, t_emb,c_emb], dim=-1)
        for i, l in enumerate(self.linear):
            h = self.linear[i](h)
            h = F.relu(h)
            if i in self.skips:
                h = torch.cat([x_emb, t_emb,c_emb, h], -1)
...
class DeformNetwork_MIX_V2(nn.Module):
    def __init__(self, D=8, W=256, input_ch=3, output_ch=59, x_multires=10, t_multires=10):
        super(DeformNetwork_MIX_V2, self).__init__()
        self.D = D
        self.W = W
        self.input_ch = input_ch
        self.output_ch = output_ch
        ...
        self.embed_time_fn, time_input_ch = get_embedder(self.t_multires, 7)
        self.embed_fn, xyz_input_ch = get_embedder(self.x_multires, 3)
        self.input_ch = xyz_input_ch + time_input_ch
...
class DeformNetwork_MIX_V3(nn.Module):
    def __init__(self, D=8, W=256, input_ch=3, output_ch=59, x_multires=10, t_multires=10):
        super(DeformNetwork_MIX_V3, self).__init__()
        self.D = D
        self.W = W
        self.input_ch = input_ch
        self.output_ch = output_ch
        ...
        self.embed_time_fn, time_input_ch = get_embedder(self.t_multires, 3)
        self.embed_fn, xyz_input_ch = get_embedder(self.x_multires, 3)
        self.input_ch = xyz_input_ch + time_input_ch
...
class DeformNetwork_MIX_V4(nn.Module):
    def __init__(self, D=8, W=256, input_ch=3, output_ch=59, x_multires=10, t_multires=10):
        super(DeformNetwork_MIX_V4, self).__init__()
        self.D = D
        self.W = W
        self.input_ch = input_ch
        self.output_ch = output_ch
