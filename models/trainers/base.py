...
class BasicTrainer(nn.Module):
    ...
    def freeze_weight(self,ref_flag):
        if ref_flag:
            for model in self.models.values():
                model.requires_grad_(False)
            self.models['Background'].deform_network_ref.requires_grad_(True)
        else:
            for model in self.models.values():
                model.requires_grad_(True)
            self.models['Background'].deform_network_ref.requires_grad_(False)
    ...
    def initialize_optimizer_ori(self) -> None:
        # get param groups first
        self.param_groups = {}
        for class_name, model in self.models.items():
            self.param_groups.update(model.get_param_groups())
        ...
        self.optimizer_ori = torch.optim.Adam(groups, lr=0.0, eps=1e-15)
        self.lr_schedulers_ori = lr_schedulers
        # self.grad_scaler = torch.cuda.amp.GradScaler(enabled=self.optim_general.get("use_grad_scaler", False))
   
    def initialize_optimizer_ref(self) -> None:
        # get param groups first
        self.param_groups = {}
        
        self.param_groups.update(self.models['Background'].get_param_groups_ref())
    ...
    def initialize_optimizer_ref_bak(self) -> None:
        # get param groups first
        self.param_groups = {}
        ...
        self.optimizer_ref = torch.optim.Adam(groups, lr=0.0, eps=1e-15)
        self.lr_schedulers_ref = lr_schedulers
        # self.grad_scaler = torch.cuda.amp.GradScaler(enabled=self.optim_general.get("use_grad_scaler", False))
    
    def initialize_optimizer_bak(self) -> None:
        # get param groups first
        self.initialize_optimizer_ori()
        self.initialize_optimizer_ref_bak()
        self.grad_scaler = torch.cuda.amp.GradScaler(enabled=self.optim_general.get("use_grad_scaler", False))
    
    def set_optimizer(self,ref_flag):
        if ref_flag:
            self.optimizer = self.optimizer_ref
            self.lr_schedulers = self.lr_schedulers_ref
        else:
            self.optimizer = self.optimizer_ori
            self.lr_schedulers = self.lr_schedulers_ori
        self.freeze_weight(ref_flag)
    ...
    def compute_losses(
        ...
        if "egocar_masks" in image_infos:
            ...
            # valid_loss_mask = image_infos['dynamic_masks']
        ...
        loss_dict.update({
            "rgb_loss": self.losses_dict.rgb.w * Ll1*loss_weight,
            "ssim_loss": self.losses_dict.ssim.w * simloss * loss_weight,
        })
        ...
        # depth loss
        if not ref_flag and self.depth_loss_fn is not None:
    ...
    def load_state_dict(self, state_dict: dict, load_only_model: bool =True, strict: bool = True,step: int = None):
        if step is None:
