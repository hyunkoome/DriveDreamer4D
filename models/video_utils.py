...
def render(
    ...
    """
    # rgbs
    rgbs, gt_rgbs, rgb_sky_blend, rgb_sky,gs_rgbs = [], [], [], [],[]
    ...
    with torch.no_grad():
        ...
        for i in tqdm(indices, desc=f"rendering {dataset.split}", dynamic_ncols=True):
            ...
            if interp is not None:
                ...
                image_infos['img_idx']*=(interp+1)
                image_infos['frame_idx']*=(interp+1)
                image_infos['normed_time'][:]=i*(interp+1)/(num_frames*(interp+1)-interp-1)
            ...
            # render the image
            results = trainer(image_infos, cam_infos,ref_flag=ref_flag)
            ...
            for k, v in image_infos.items():
                if isinstance(v, Tensor) and "pixels" in k:
                    image_infos[k] = v.clamp(0., 1.)
            ...
            gs_rgb = results['rgb_gaussians']
            gs_rgbs.append(get_numpy(gs_rgb))
            ...
            if compute_metrics:
                ...
                logger.info(f"Frame {idx}: PSNR {psnr:.4f}, SSIM {ssim_score:.4f}")
                psnrs.append(psnr)
