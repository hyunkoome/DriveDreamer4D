...
# sys.path.insert(1,'/mnt/pfs/users/guosheng.zhao/1-code/1-drivestudio')
...
def setup(args):
    ...
    def load_model_hf(repo_id, filename, ckpt_config_filename, device='cpu'):
        ...
        cache_config_file = os.path.join('/mnt/pfs/models/huggingface/models--ShilongLiu--GroundingDINO',ckpt_config_filename)
        args = SLConfig.fromfile(cache_config_file) 
        model = build_model(args)
        args.device = device

        # cache_file = hf_hub_download(repo_id=repo_id, filename=filename)
        cache_file = os.path.join('/mnt/pfs/models/huggingface/models--ShilongLiu--GroundingDINO',filename)
    ...
    sam = build_sam(checkpoint='/mnt/pfs/users/guosheng.zhao/1-code/1-drivestudio/sam_weight/sam_vit_h_4b8939.pth')
    sam.cuda()
...
def add_to_mask_dict(masks_dict, mask_path):
    ...
    if frame not in masks_dict:
        masks_dict[frame] = [None] * 5 # FRONT_LEFT, FRONT, FRONT_RIGHT 1, 0, 2
    if cam == 1:
        ...
        masks_dict[frame][3] = mask
    elif cam == 4:
        masks_dict[frame][4] = mask
    


def segment_with_text_prompt(datadir, BOX_TRESHOLD=[0.3]*5, TEXT_TRESHOLD=0.25, ignore_exists=True):
    save_dir = os.path.join(datadir, 'sky_masks')
    ...
    for image_path in tqdm(image_files):
        ...
        image_base_name = image_base_name.replace('.jpg','.png')
...
if __name__ == "__main__":
    ...
    data_root = '/mnt/pfs/users/guosheng.zhao/1-code/1-drivestudio/data/waymo/processed/validation'
    split_file = open('/mnt/pfs/users/guosheng.zhao/1-code/1-drivestudio/data/waymo_val_scenes.txt', "r").readlines()[1:]
    scene_ids_list = [line.strip().split(",")[0] for line in split_file]
    # data_dirs =[ ]
    for id in tqdm(scene_ids_list):
        fn = id.zfill(3)
        data_dir = os.path.join(data_root,fn)
    ...
    # track_parallel_progress(segment_with_text_prompt,data_dirs,8)
